/*
 * Exercise 10.19 对一个序列的字符串进行字典排序，去重操作，然后按照大小进行重排，
 * 打印输出比接收的长度参数长的字符串，用lambda表达式实现,用stable_partition替换partition
 * */
#include <iostream>
#include <vector>
#include <string>
...
void biggies(vector<string> &v_str, const string::size_type size) {
    ...
    // stable_partition会将符合条件的放在前部分，不符合条件的放在后部分,顺序依然保持一致
    auto iterator_last = stable_partition(v_str.begin(), v_str.end(),
